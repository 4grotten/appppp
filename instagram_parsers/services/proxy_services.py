...
class InstagramClientService:
    ...
    def get_anon_client(cls, for_getting_username: bool = False) -> Client:
        proxy = ProxyService.get_random_formed_proxy(
            for_getting_username=for_getting_username
        )
...
class ProxyService:
    @classmethod
    def get_random_formed_proxy(
        cls, for_getting_username: bool = False
    ) -> Optional[str]:
        proxies = Proxy.objects.filter(
            for_getting_username=for_getting_username, expires_at__gt=now()
        )
        logging.info(f"Proxies available: {proxies.count()}")
        if proxies:
            proxies_info = [proxy.login for proxy in proxies]
            logging.info(f"Available proxies: {proxies_info}")
    ...
    def get_random_proxy_for_requests(cls, for_getting_username: bool = False):
        ...
        if proxies:
            ...
            list_proxies = [
                dict(https=f"http://{proxy.login}:{proxy.password}@{proxy.http_s}")
            ]
            return list_proxies
        return None
...
class LoginDeviceService:
    ...
    def get_random_login_settings(cls):
        login_settings = LoginDevice.objects.filter(
            for_getting_username=True, is_broke=False
        )
