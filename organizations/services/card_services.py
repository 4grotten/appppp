...
class DiscountCardService:
    ...
    def create_or_reactivate(
        cls, organization: Organization, percent: int, type: str, **kwargs
    ):
        reactivated = DiscountCard.objects.filter(
            is_published=False, organization=organization, percent=percent, type=type
    ...
    def get_grouped_discounts(cls, organization_id: int) -> dict:
        discounts = DiscountCard.objects.filter(
            organization_id=organization_id, is_published=True
        )
        discounts_dict = {
        ...
        if (
            len(discounts_dict[DiscountCard.CASHBACK]) > 1
            and discounts_dict[DiscountCard.CASHBACK][0].percent == 0
        ):
            discounts_dict[DiscountCard.CASHBACK] = discounts_dict[
                DiscountCard.CASHBACK
            ][1:]
    ...
    def delete_discount(cls, discount: DiscountCard, user: User) -> DiscountCard:
        if not OrganizationService.user_can_edit_organization(
            organization=discount.organization, user=user
        ):
    ...
    def swap_or_update(cls, discount: DiscountCard, percent: Union[int, None]):
        ...
        temp = 999
        updated = DiscountCard.objects.filter(
            is_published=False, percent=percent
        ).update(percent=temp)
        ...
        DiscountCard.objects.filter(is_published=False, percent=temp).update(
            percent=old_value
        )
        ...
        if cashbacks:
            transaction.on_commit(
                lambda: send_notifications_to_all_users.delay(
                    ...
                    extra_data=dict(
                        cashback=str_cashback, address=organization.address
                    ),
                )
            )
    ...
    def organize_cumulative_cards(cls, organization: Organization):
        ...
        organization.discounts.filter(type=DiscountCard.CUMULATIVE).update(
            next_cumulative=None
        )

        cumulative_cards = organization.discounts.filter(
            is_published=True, type=DiscountCard.CUMULATIVE
        ).order_by("limit")
    ...
    @classmethod
    def get_fixed_discounts_of_organization(
        cls, organization: Organization
    ) -> QuerySet:
        return DiscountCard.objects.filter(
            organization=organization, type=DiscountCard.FIXED, is_published=True
        )

    @classmethod
    def get_cashback_discounts_of_organization(
        cls, organization: Organization
    ) -> QuerySet:
        return DiscountCard.objects.filter(
            organization=organization,
            type=DiscountCard.CASHBACK,
            is_published=True,
            percent__gt=0,
        )
    ...
    @classmethod
    def get_lowest_cumulative_card(
        cls, organization: Organization
    ) -> Union[DiscountCard, None]:
        return (
            organization.discounts.filter(
    ...
    def bulk_delete_discounts(
        cls, cards: list, organization: Organization, deleted_by: User
    ):
        should_organize = False
        ...
        for card in cards:
            if not card.organization == organization:
                raise NotAcceptableException(
                    _("Card does not belong to this organization")
                )
    ...
    def bulk_update_discounts(
        cls, cards_data: list, organization: Organization, updated_by: User
    ):
        should_organize = False
        percents = list()
        cashbacks = list()
    ...
    @classmethod
    def get_unique_discount_percents_to_display(cls, organization: int) -> list:
        values = (
            DiscountCard.objects.filter(
                is_published=True, organization_id=organization, percent__gt=0
            )
            .distinct("percent")
            .order_by("percent")
    ...
    def get_max_discount(cls, organization: Organization) -> int:
        max_discount = (
            DiscountCard.objects.filter(is_published=True, organization=organization)
            .distinct("percent")
            .order_by("-percent")
            .values_list("percent", flat=True)[:1]
        )
        if max_discount:
    ...
    def update_discount_currency(cls, organization: Organization, new_currency: str):
        rate = CurrencyConverterService.get_rate(
            from_currency=organization.currency.code, to_currency=new_currency
        )
        DiscountCard.objects.filter(
            organization=organization, type=DiscountCard.CUMULATIVE
