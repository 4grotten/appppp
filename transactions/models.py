...
from common.utils import DecimalDecoder, DecimalEncoder, upload_file_with_unique_name
...
class Transaction(TimestampModel):
    ...
    WITHDRAWAL_TYPES = ((BANKCARD, BANKCARD), (SWIFT, SWIFT))
    ...
    client = models.ForeignKey(
        User, on_delete=models.PROTECT, related_name="bought_transactions", null=True
    )
    processed_by = models.ForeignKey(
        User, on_delete=models.PROTECT, related_name="processed_transactions", null=True
    )
    organization = models.ForeignKey(
        Organization, on_delete=models.PROTECT, null=True, related_name="transactions"
    )
    ...
    currency = models.ForeignKey(
        Currency, on_delete=models.PROTECT, related_name="transactions", default="KGS"
    )
    ...
    fixed_cart = models.JSONField(
        null=True, encoder=DecimalEncoder, decoder=DecimalDecoder
    )
    ...
    withdrawal_type = models.CharField(
        choices=WITHDRAWAL_TYPES, max_length=20, default=BANKCARD
    )

    discount_type = models.CharField(
        choices=DISCOUNT_TYPES, max_length=20, default=MANUAL
    )
    type = models.CharField(choices=TYPE, max_length=20, default=OFFLINE)
    payment_status = models.CharField(
        choices=PAYMENT_STATUS, max_length=20, default=IN_PROGRESS
    )
    delivery_type = models.CharField(
        choices=DELIVERY_TYPE, max_length=20, default=SELF_PICKUP
    )
    ...
    files = models.ManyToManyField(
        TransactionFile, blank=True, related_name="transactions"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.product_coupon:
            self.original_amount -= self.product_coupon
        if self.discount_coupon:
            self.final_amount = self.final_amount - (
                self.final_amount * self.discount_coupon / 100
            )
...
class Recipient(models.Model):
    payout_system = models.ForeignKey(
        PayoutSystem, on_delete=models.PROTECT, related_name="recipients"
    )
    image = models.ForeignKey(
        "common.File", on_delete=models.SET_NULL, null=True, blank=True
    )
...
class Balance(models.Model):
    KGS = "KGS"
    TRC = "TRC20"
    CURRENCY = ((KGS, KGS), (TRC, TRC))
    organization = models.ForeignKey(
        Organization, on_delete=models.PROTECT, related_name="balances"
    )
