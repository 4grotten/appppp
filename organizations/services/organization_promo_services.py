...
class OrganizationPromoService:
    ...
    def get_promo_stats_for_user(cls, organization_id: int, user: User) -> dict:
        ...
        if not OrganizationService.user_can_edit_organization(
            ...
            raise PermissionDeniedException(_("No rights to get promo details"))
        return {"subscribers_count": organization.promo_subscribers.count()}

    @classmethod
    @transaction.atomic
    def create_organization_promo(
        cls,
        user: User,
    ...
    @classmethod
    @transaction.atomic
    def update_organization_promo(
        cls,
        organization_promo: OrganizationPromo,
        total_cashback: Decimal,
        cashback: Decimal,
        image: File,
        changed_by: User,
        ...
        try:
            organization_promo.total_cashback = (
                organization_promo.granted_amount + total_cashback
            )
            organization_promo.cashback = cashback
            organization_promo.image = image
            organization_promo.save()
            PromoEditLogService.record_action(
                promo=organization_promo, changed_by=changed_by
            )
            return organization_promo
        except Exception as e:
            raise IntegrityException(
                _("Can not update organization_promo: {}").format(str(e))
            )

    @classmethod
    def get_usable_promo(
    ...
    @classmethod
    def get_usable_promo_from_json(cls, organization: int) -> OrganizationPromo:
        promo = OrganizationPromo.objects.filter(organization_id=organization).first()
        if promo is None:
            return None
        if promo.total_cashback - promo.granted_amount >= promo.cashback:
            return promo
    ...
    @classmethod
    def get_available_promo_cashback_amount_from_json(
        cls, organization: int
    ) -> Optional[Decimal]:
        promo = cls.get_usable_promo()

    @classmethod
    def get_active_promos(cls) -> QuerySet:
        return OrganizationPromo.objects.filter(
            cashback__lte=F("total_cashback") - F("granted_amount")
        ).exclude(
    ...
    def get_types_with_active_promos(cls, country: Optional[str] = None) -> QuerySet:
        ...
        if country:
            active_promos_query = active_promos_query.filter(
                organization__country__code=country
            )

        organization_ids = active_promos_query.values_list("organization_id", flat=True)

        return (
            OrganizationType.objects.filter(organizations__id__in=organization_ids)
            .distinct()
            .order_by("title")
        )

    @classmethod
    def get_filtering_promos_by_country(
        cls, country: Optional[str] = None, type_id: Optional[int] = None
    ) -> QuerySet:
...
class PromoEditLogService:
    ...
    def record_action(cls, promo: OrganizationPromo, changed_by: User):
        role = OrganizationService.get_user_role_in_organization(
            organization=promo.organization, user=changed_by
        )
        cls.create(
            promo=promo,
            changed_by=changed_by,
            employee_name=changed_by.full_name,
            employee_role=role,
            employee_avatar=changed_by.avatar,
        )
...
class PromoSubscriberService:
    ...
    def use_promo_for_new_subscriber(cls, organization: Organization, follower: User):
        ...
        if promo is None:
            return
        try:
            PromoSubscriber.objects.create(
                organization=organization, subscriber=follower, cashback=promo.cashback
            )
            promo.granted_amount = F("granted_amount") + promo.cashback
            promo.save()
            promo.refresh_from_db()
            OrganizationClientFinancialStatusService.change_accrued_cashback(
                user=follower, organization=organization, change_amount=promo.cashback
            )
        except IntegrityError:
            return
    ...
    def user_has_promo_cashback(cls, organization: Organization, user: User):
        return PromoSubscriber.objects.filter(
            organization=organization, subscriber=user
        ).exists()
