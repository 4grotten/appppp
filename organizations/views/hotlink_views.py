...
class HotlinkListCreateView(ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return HotlinkService.get_hotlinks(
...
class HotlinkRetrieveUpdateDestroyView(RetrieveUpdateDestroyAPIView):
    ...
    def get_object(self):
        ...
        return HotlinkService.get_editable_hotlink_for_user(
            hotlink_id=self.kwargs["pk"], user=self.request.user
        )
...
class OrganizationHotlinkShopItems(ListAPIView):
    ...
    def get_queryset(self):
        ...
        return ShopItemService.get_organization_items_queryset_for_user(
            organization=organization, user=self.request.user, search=search
        ).order_by("-updated_at")
...
class HotlinkItemsListUpdateView(GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        queryset = HotlinkService.get_hotlink_collection_items(
            hotlink=hotlink, user=self.request.user
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        HotlinkService.edit_hotlink_selected_items(
            hotlink=hotlink,
            added=serializer.validated_data["added"],
...
class HotlinkSubcategoriesListUpdateView(GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        hotlink = HotlinkService.get(id=self.kwargs["pk"], link_type=HOTLINK_COLLECTION)
        queryset = HotlinkService.get_hotlink_collection_subcategories(
            hotlink=hotlink, user=self.request.user
        )
...
class HotlinkSelectedSubcategoriesListView(ListAPIView):
    ...
    def get_queryset(self):
        hotlink = HotlinkService.get(id=self.kwargs["pk"], link_type=HOTLINK_COLLECTION)
        return HotlinkService.get_selected_subcategories_in_hotlink_collection(
            hotlink=hotlink
        )
...
class CollectionLinksCreateView(CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        HotlinkCollectionLinkService.create_collection_link(
            hotlink=serializer.validated_data["hotlink"],
            content=serializer.validated_data["content"],
            user=request.user,
        )

        return Response(
            data={"message": _("Successfully created")}, status=status.HTTP_201_CREATED
        )
...
class CollectionLinksListView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        return HotlinkCollectionLinkService.get_collection_links(
            hotlink=hotlink, user=self.request.user
        )
...
class CollectionLinkUpdateDestroyView(DestroyAPIView):
    ...
    def delete(self, request, *args, **kwargs):
        instance = HotlinkCollectionLinkService.get(id=kwargs["pk"])
        HotlinkCollectionLinkService.delete_collection_link(
            collection_link=instance, user=request.user
        )
