...
class ZinaPayPreprocessSerializer(serializers.Serializer):
    ...
    amount = serializers.DecimalField(
        required=True, max_digits=12, decimal_places=2, min_value=0.01
    )

    def validate_organization(self, value):
        from organizations.services.zinapay_service import ZinaPayService
        if not ZinaPayService.is_configured(value):
            raise serializers.ValidationError(_("ZinaPay is not configured for this organization"))
        return value
    ...
    def validate_currency(self, value):
        ...
        code = value.upper()
        try:
            ZinaPayService.validate_currency(code)
        except BadRequestException as e:
            # This ensures we don't go further with unsupported currency
