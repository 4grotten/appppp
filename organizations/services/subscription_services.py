...
from notifications.constants import (
    ...
    FOLLOWED_TO_ORGANIZATION_TITLE,
    ORGANIZATION_FOLLOWED_TYPE,
    ORGANIZATION_FOLLOWED_TITLE,
    SUBSCRIPTION_NOTIFICATION_DESCRIPTION,
    NOTIFICATION_MODE_PERSONAL,
    BG_FOLLOWED_TO_ORGANIZATION_DESCRIPTION,
    BG_ORGANIZATION_FOLLOWED_DESCRIPTION,
    BG_FOLLOWED_TO_ORGANIZATION_DESCRIPTION_RU,
    BG_ORGANIZATION_FOLLOWED_DESCRIPTION_RU,
    BG_FOLLOWED_TO_ORGANIZATION_DESCRIPTION_DE,
    BG_ORGANIZATION_FOLLOWED_DESCRIPTION_DE,
    BG_FOLLOWED_TO_ORGANIZATION_DESCRIPTION_TR,
    BG_FOLLOWED_TO_ORGANIZATION_DESCRIPTION_ZH,
    BG_ORGANIZATION_FOLLOWED_DESCRIPTION_TR,
    BG_ORGANIZATION_FOLLOWED_DESCRIPTION_ZH,
)
...
class SubscriptionService:
    ...
    def get_number_of_subscriptions(cls, organization: Organization) -> int:
        return Subscription.objects.filter(
            organization=organization, status="subscribed"
        ).count()

    @classmethod
    def toggle_subscription_status(cls, organization: Organization, user: User) -> str:
        if (
            organization.is_private is True
            and not OrganizationService.user_can_edit_organization(
                organization=organization, user=user
            )
            and cls.is_subscribed(organization, user) == "not_subscribed"
        ):
            Subscription.objects.create(
                organization=organization, user=user, status="pending"
            )
            return "pending"
        elif (
            organization.is_private is True
            ...
            and cls.is_subscribed(organization, user) == "pending"
        ):
            ...
            if created:
                PromoSubscriberService.use_promo_for_new_subscriber(
                    organization=organization, follower=user
                )
                ...
                sent_notification.delay(
                    ...
                    title=ORGANIZATION_FOLLOWED_TITLE.format(
    ...
    def get_user_subscriptions(cls, user: User) -> QuerySet:
        if user.is_authenticated:
            organizations = (
                Organization.active_organizations.filter(
                    id__in=user.subscriptions.values("organization_id")
                )
                .annotate(
    ...
    def get_organization_followers(
        cls, organization_id: int, user: User = None
    ) -> QuerySet:
        ...
        if organization.show_followers:
            if user:
                ...
                return User.objects.filter(
                    ...
                    & Q(subscriptions__status="subscribed")
                ).order_by("-subscriptions__id")
            return User.objects.filter(
                Q(subscriptions__organization=organization)
                & Q(subscriptions__status="subscribed")
            ...
            if user:
                if OrganizationService.user_can_edit_organization(
                    organization=organization, user=user
                ) and OrganizationService.user_can_see_stats(
                    organization=organization, user=user
                ):
                    return User.objects.filter(
                        subscriptions__organization_id=organization_id
    ...
    @classmethod
    def get_organization_blocked_users(
        ...
        if OrganizationService.user_can_edit_organization(
            ...
            return User.objects.filter(
                blocked_user__organization_id=organization_id
            ).order_by("-blocked_user__id")
    ...
    def accept_all_followers(cls, organization: Organization):
        ...
        for i in subscriptions:
            PromoSubscriberService.use_promo_for_new_subscriber(
                organization=organization, follower=i.user
            )
            ...
            sent_notification.delay(
                ...
                organization_id=organization.id,
                extra_data=dict(
                    org_title=organization.title, address=organization.address
                ),
            )
...
class UserOrgSubscriptionService:
    ...
    @classmethod
    def create_user_org_subscription(
        cls,
        user: User,
        processed_by: User,
        ...
        discount_percent = (
            Decimal(promocode.discount_percent) if promocode else Decimal("0")
        )
        profit_percent = (
            Decimal(promocode.profit_percent) if promocode else Decimal("0")
        )
        tax = tariff.country.invoice_info.tax
        tax_decimal = Decimal(str(tax)) / Decimal("100")
        final_price = total_price * (Decimal("1") - discount_percent / Decimal("100"))
        final_price = (final_price * tax_decimal) + final_price
        ...
        with db_transaction.atomic():
            ...
            if promocode:
                ...
                profit_usdt = CurrencyConverterService.convert(
    ...
    @classmethod
    def create_user_org_subscription_transaction(
        cls,
        user: User,
        processed_by: User,
        tariff: RegionalTariff,
