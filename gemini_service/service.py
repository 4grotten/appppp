...
class GeminiAIService:
    ...
    LANGUAGE_RULE = (
        "Языковое правило: всегда определяй язык входного текста пользователя и отвечай строго на этом же языке. "
        "Ничего не переводи на другой язык без явной просьбы. "
        "Если вход смешанный, используй язык первой осмысленной фразы."
    )
    ...
    PROMPT_TEMPLATES = {
        "item_description": (
            ...
            "Твоя задача: Написать продающее, лаконичное описание товара на основе входных данных. "
            "Ответ строго 2–3 предложениями, без markdown, без звездочек, без списков и без лишней воды. "
            "Можно использовать 1–2 уместных эмодзи. "
            "Всегда соблюдай языковое правило."
        ),
        "prompt": (
        ...
        "price_prompt": (
            ...
            "на 3 строки без подробностей и мета описаний, не используй звездочки. "
            "Всегда соблюдай языковое правило."
        ),
        "discount_prompt": (
